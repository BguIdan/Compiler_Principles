...
(define buildCiscConstTable 
	(lambda (constTable)
		...
		(string-append "PUSH (" (number->string (length constTable)) ");" n (call-malloc)
...
(define code_gen_if3 (lambda (test dit dif code_gen major) 
	...
	(string-append  (code_gen test major) n
					"CMP (R0, SOB_FALSE);" n
...
(define code_gen_or (lambda (exprs code_gen major label-string)
	(if (null?  exprs) 
		(string-append label-string ":" n)
		(string-append 
			(code_gen (car exprs)) n
			"CMP (R0, SOB_FALSE);" n
			"JUMP_NE (" label-string ");" n
			(code_gen_or (cdr exprs) code_gen label-string)))))
...
(define applic_label 
	(lambda ()
		(string-append "L_Error_cannot_apply_non_closure_" (number->string (updateCounter)))
	))
...
(define code_gen_applic_push_args 
	(lambda (argsLst)
		(if (null? argsLst) 
			""
			(string-append "PUSH (" (code_gen (car argsLst)) ");" n 
				(code_gen_applic_push_args (cdr argsLst))))))

(define push_args_if_needed
	(lambda (argLst)
		(if (null? argsLst)
			""
			(string-append (code_gen_applic_push_args argsLst)
						    "PUSH (" (length argsLst) ");" n) 
		)))
...
; not implemented this is the same as regular applic
...
(define code_gen_lambda_simple
	(lambda (newMajor args body code_gen)
		(string-append
			"MOV (R1,FPARG(0));" n
			"PUSH(" newMajor ");" n "CALL(MALLOC);" n "DROP(1);" n
			"MOV (R2,R0);" n
			"MOV (INDD(R3,0),R2);" n
			...
			"MOV (INDD(R0 ,1) , R2);" n
			(createLambdaBodyInCISC body code_gen)
			)
		))
	...
	(lambda (exp major)
		...
		(compose-patterns
				...
				`(bvar ,(? 'var-name) ,(? 'major) ,(? 'minor)) 
					(lambda (var-name major minor) 
						(string-append 
							...
							"MOV (R0,FPARG(0));" n 
							"MOV (R0,INDD(R0," major "));" n
							"MOV (R0,INDD(R0," minor "));" n
			...
			(pattern-rule
				`(if3 ,(? 'test) ,(? 'dit) ,(? 'dif))
					(lambda (test dit dif) 
						(string-append (CISC_comment "if3 code starts here")
...
SHOW (\"num\",INDD ((INDD (CONSTARRAY,4)),1));
SHOW (\"num\",INDD ((INDD (CONSTARRAY,5)),1));
SHOW (\"num\",INDD ((INDD (CONSTARRAY,6)),1));
SHOW (\"num\",INDD ((INDD (CONSTARRAY,7)),1));


SHOW (\"car\",INDD ((INDD (CONSTARRAY,8)),1));
SHOW (\"cdr\",INDD ((INDD (CONSTARRAY,8)),2));

SHOW (\"car\",INDD ((INDD (CONSTARRAY,9)),1));
SHOW (\"cdr\",INDD ((INDD (CONSTARRAY,9)),2));

SHOW (\"car\",INDD ((INDD (CONSTARRAY,10)),1));
SHOW (\"cdr\",INDD ((INDD (CONSTARRAY,10)),2));

SHOW (\"car\",INDD ((INDD (CONSTARRAY,11)),1));
SHOW (\"cdr\",INDD ((INDD (CONSTARRAY,11)),2));
...
(define a (lambda (src)
	(let* ((after_ass3 (runAss3 (parse src)))
			(CISC-const-table (buildConstantTable after_ass3))
			(codeGen (code_gen after_ass3))
		   )
