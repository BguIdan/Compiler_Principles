...
(define code_gen_if3 (lambda (test dit dif code_gen) 
	(let ((else_label (if_label #t))
		 (exit_label (if_label #f)))
	(string-append  (code_gen test) n
					"CMP (R0, SOB_FALSE);" n
...
(define code_gen_seq (lambda (exprs code_gen)
		(if (null? exprs) ""
			(string-append (code_gen (car exprs)) (code_gen_seq (cdr exprs) code_gen)))))
...
(define  code_gen  
	(lambda (exp)
		...
		;			`(bvar ,(? 'var-name) ,(? 'major) ,(? 'minor)) 
			...
			(pattern-rule `(const ,(? 'const)) 
						(lambda (const) (code_gen_constant const code_gen)))
			...
			(pattern-rule
				`(if3 ,(? 'test) ,(? 'dit) ,(? 'dif))
					(lambda (test dit dif) 
						...
						(code_gen_if3 test dit dif code_gen))
						(CISC_comment "if3 code ends here")))

			(pattern-rule
				`(or  ,(? 'expr))
					(lambda (exprs) (string-append (CISC_comment "or code starts here")
						(code_gen_or exprs code_gen (string-append "L_OR_EXIT" (number->string (updateCounter))))
			...
			(pattern-rule
				`(seq ,(? 'exps))
					(lambda (exprs) (string-append (CISC_comment "seq code starts") 
						(code_gen_seq exprs code_gen) (CISC_comment "seq code ends"))))
